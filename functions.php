...
// Scripts del tema
add_action( 'wp_enqueue_scripts', 'ovscripts' );
function ovscripts() {
	$template_url = get_bloginfo( 'template_url' );

	wp_register_script( 'jquerycustom', $template_url .'/js/jquery-ui-1.10.4.custom.min.js', array('jquery'), '', 1);
	wp_register_script( 'bootstrap', $template_url .'/js/bootstrap.js', array('jquery'), '', 1);
	wp_register_script( 'gsdkcheckbox', $template_url .'/js/gsdk-checkbox.js', array('jquery'), '', 1);
	wp_register_script( 'gsdkradio', $template_url .'/js/gsdk-radio.js', array('jquery'), '', 1);
	wp_register_script( 'gsdkbootstrapswitch', $template_url .'/js/gsdk-bootstrapswitch.js', array('jquery'), '', 1);
	wp_register_script( 'getshitdone', $template_url .'/js/get-shit-done.js', array('jquery'), '', 1);
	wp_register_script( 'custom', $template_url .'/js/custom.js', array('jquery'), '', 1);
	wp_register_script( 'themejs', $template_url .'/js/theme.js', array('jquery'), '', 1);

	wp_enqueue_script( 'jquerycustom' );
	wp_enqueue_script( 'themejs' );
	wp_enqueue_script( 'bootstrap' );
	wp_enqueue_script( 'gsdkcheckbox' );
	wp_enqueue_script( 'gsdkradio' );
	wp_enqueue_script( 'gsdkbootstrapswitch' );
	wp_enqueue_script( 'getshitdone' );
	wp_enqueue_script( 'custom' );
}


// Remover AIM y etc de usuarios suscriptores
...
* ------------------------------------------------------------- */
 function soloUsuarioRegistrado() 
 {
 	...
 	$registrado = get_user_meta( $user_ID, 'registradoApi', true); 
 	...
 	if (is_user_logged_in())
 	{
		...
		if ($registrado) 
		{	
			//Si está en completar registro lo envío al escritorio
			if (is_page_template('sh-page-registro-api.php'))
			{	
		...
		if (!$registrado) {
			...
			//Si No está en completar registro lo envío al escritorio
			if (!is_page_template('sh-page-registro-api.php'))
			{	
				wp_redirect( site_url( '/registro-api/' ) ); exit;
			}
		}
		if (is_home()) 
		{
 	...
 	{
 		// Si no es Home-Login lo envía a home
 		if (!is_home()) 
		{
 ...
 /* Redirige al formulario de home si tratan de ingresar por /wp-admin
* ------------------------------------------------------------- */
...
function datosApi() {
	...
	$userData['calle']			= $userMeta['calleAPi'][0];
	$userData['colonia']		= $userMeta['coloniaApi'][0];
	$userData['ciudad']			= $userMeta['ciudadApi'][0];
	$userData['estado']			= $userMeta['estadoApi'][0];
	$userData['cp']				= $userMeta['cpApi'][0];
	$userData['pais']			= $userMeta['paisApi'][0];
	// Datos bancarios
	$userData['banco']			= $userMeta['bancoApi'][0];
	$userData['clabe']			= $userMeta['clabeApi'][0];
	$userData['paypal']			= $userMeta['paypalApi'][0];
	$userData['rfc']			= $userMeta['rfcApi'][0];
	// Redes sociales
	$userData['fb']				= $userMeta['fbApi'][0];
	$userData['tw']				= $userMeta['twApi'][0];
	$userData['ld']				= $userMeta['ldApi'][0];
	$userData['gp']				= $userMeta['gpApi'][0];
	//Planes
	...
	//Movimientos
	$userData['fpbasico']		= $userMeta['fechaPagoBasicoApi'][0];
	$userData['fppremier']		= $userMeta['fechaPagoPremierApi'][0];
	$userData['fpmantenimiento']= $userMeta['fechaPagoMantenimientoApi'][0];
